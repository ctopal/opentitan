...
class aon_timer_smoke_vseq extends aon_timer_base_vseq;
  ...
  task smoke_configure();
    ...
    `uvm_info(`gfn, $sformatf("\n\t ----| Writing random COUNT value of %d to WKUP", wkup_count), UVM_LOW)
  //  csr_utils_pkg::csr_rd(.ptr(ral.wkup_count), .value(wkup_count));
  //  `uvm_info(`gfn,
  //            $sformatf("\n\t ---| reading 0x%0h from wkup_count", wkup_count),
  //            UVM_LOW)

    csr_utils_pkg::csr_wr(ral.wdog_count, wdog_count);
    `uvm_info(`gfn, $sformatf("\n\t ----| Writing random COUNT value of %d to WDOG", wdog_count), UVM_LOW)
  //  csr_utils_pkg::csr_rd(.ptr(ral.wdog_count), .value(wdog_count));
  //  `uvm_info(`gfn,
  //            $sformatf("\n\t ---| reading 0x%0h from WDOG COUNT", wdog_count),
  //            UVM_LOW)
